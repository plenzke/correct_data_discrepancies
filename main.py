...
def find_name(text, initials): # Функция для поиска ФИО в тексте на основе созданного паттерна
    ...
    if not text_is_upper(text):
        ...
        if name:
            ...
            # КОСТЫЛЬ Если не найдено имя с инициалами со столбца "Наименование счета" то применяем общий паттерн без уточнения с инициалом имени
            common_pattern = fr'(([А-Я][^A-Я\s\.\,\/][a-я]*)(\s+)([А-Я][a-я]*)' + \
              fr'(\.*\s*|\s+|\B)([А-Я]?\.*[a-я]*)?)'
            name = re.findall(common_pattern, text)
            return name[0][0]
    else:
        try:
            start_number_index = re.search(
                ...
                # В данном случае написано только для РВПС, автоматизация поиска частовстречающихся до имени слов из документа еще не реализовано
                text, flags=re.I).end()
